...
class DialogueEnv():
    ...
    def __init__(self):
        ...
        # da
        self.da_df = pd.read_csv(self.params.get('path_da_for_UIpred'), encoding="shift-jis")

        # action
        self.action_df = pd.read_csv(self.params.get('path_class_name'), encoding="shift-jis")
    ...
    def getSimpleDAFromSysUtterance(self, sys_utterance):
        df = pd.read_csv(self.params.get('path_simple_da'), encoding="utf-8")
    ...
    def getDAFromSysUtterance(self, sys_utterance):
        #df = pd.read_csv(self.params.get('path_simple_da'), encoding="shift-jis")
        #print(sys_utterance)
        da = self.da_df[self.da_df['speech'] == sys_utterance]['dialogue_act'].values
        pattern = r"これから.+の話をしましょう"
        if re.search(pattern, sys_utterance) != None:
            simple_da = 'op'
        else:
    ...
    def utterance_selection_softmax(self, chg_theme, theme, prob_actions, mei_cmd=True):
        ...
        if chg_theme:
            ...
            while not done:
                # 選択する
                action = np.random.choice(actions, p=self.softmax(prob_actions))
                df = pd.read_csv(self.params.get('path_utterance_by_class_named'), encoding="shift-jis")
        ...
        if mei_cmd:
            ...
            if chg_theme:
                ...
                #print(next_sysutte)
                #print(self.da_df[self.da_df['speech'] == next_sysutte])
                da = self.da_df[self.da_df['speech'] == next_sysutte]['dialogue_act'].values[0]
