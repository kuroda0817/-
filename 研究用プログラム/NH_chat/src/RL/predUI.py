...
def makeDiaDF(reaction, s_len, u_len, su_len, da):
	...
	if da in da_list:
		da_fea = np.eye(8)[da_list.index(da)]
	else:
		da_fea = np.zeros(8)
...
def predUnknown(X_test, fea_type, is_print=False):
	...
	# スケーリングデータ，分類器を読込
	scale = np.load('../refData/RL_files/modelUIpred/{}_scale.npy'.format(fea_type))
	model = pickle.load(open('../refData/RL_files/modelUIpred/{}.model'.format(fea_type), 'rb'))
...
def changePredValueRange(fusion_pred):
	...
	lowUI_before = x1 = 3.99
	lowUI_after = y1 = 3
	highUI_before  = x2 = 4.92
	highUI_after = y2 = 5

	fusion_pred_scaled = ((y2-y1)/(x2-x1)) *(fusion_pred - x1) + y1
	#fusion_pred_scaled = 5*(fusion_pred - x1) + y1
	#fusion_pred_scaled=(6/(5.23-3.95))*(fusion_pred-3.95)+1
