...
def mmd_record():
    mmd = MMDAgent(is_debug=True)
    input("準備完了")
    # utt_list = ["これまでに行った旅行先の中で特に印象に残っている場所はどこですか？",
    #             "仙台ですか！実は私も以前宮城に行ったことがあって，牛タンがおいしかったです．田中さんは仙台旅行で何か美味しいものは食べましたか？",
    #             "そうなんですね！宮城旅行は温泉宿から眺める紅葉が最高でした．田中さんは仙台でどこか観光に行かれましたか？"]
    utt_list = ["家にいるとき料理はしますか？",
                "そうなんですね．田中さんはどんな料理を作ることが多いですか？",
                "中華料理ですか．麻婆豆腐とかチンジャオロースとか，ですか？"]
    for utt in utt_list:
        mmd.say(utt)
        input()
    mmd.end()
