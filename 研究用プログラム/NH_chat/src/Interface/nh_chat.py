...
class NHChat(AbstractChat):
    ...
    def __init__(self, model="200117_c099", mode="WebCamera", user="tmp_user", 
                 turn_num=3, text=False, asr_module="google", 
                 response_time_no_word=6.0, turn_buffer=1.5, asr_conf=False, 
                 is_debug=False, is_custom=False):
        self.debug = is_debug
        ...
        if self.is_custom == True:
            self.custom_sys_utt_generator = utt_select_interface.MainProcess()
            self.custom_sys_utt_generator.start()
        ...
        while True:
            if self.openface.is_running == True:
                ...
                time.sleep(0.1)
        self.base_time = self.openface.start_time
    ...
    def process_custom_sys_utt(self):#改善後発話選択モデル(黒田)を用いた対話
        """
        utt_select_interfaceを用いて対話
        """

        is_end = False
        if self.current_turn == 0:#0ターン目処理
            file_name = "../../user_data/{0}/voice/{0}_{1}_voice"\
                .format(self.user, str(self.current_turn).zfill(3))
            sys_utt = "これから本番の収録を開始します。"
            print("system:{}".format(sys_utt))
            ...
            while True:
                if self.asr.m_turn.is_sys_turn_end == True:
                    ...
                    time.sleep(0.1)
            self.record.stop()
            user_utt,confident_noun = self.asr.read_result()
            # 録音したwavファイルが保存されるまで待機
            while True:
                if self.record.save_complete == True:
                    time.sleep(0.1)
            ...
            while True:
                # システム発話が選択されるまで待ち
                if self.custom_sys_utt_generator.is_end:
                    is_end = True
                    break
                if self.custom_sys_utt_generator.sys_utt != "":
                    break
                time.sleep(0.1)
            sys_utt = self.custom_sys_utt_generator.get_sys_utt()
            sys_utt_yomiage=sys_utt
            print("system:{}".format(sys_utt))
            pattern = re.compile(r'^({})ですか。'.format(self.confident_noun))
            if bool(pattern.search(sys_utt)):#対話行為検索のため復唱部分を削除
                sys_utt=sys_utt[sys_utt.find("。")+1:]
            if sys_utt=="":#システム発話が空なら終了
                return is_end
            da = self.env.getDAFromSysUtterance(sys_utt)#対話行為検索
            self.env.sys_utterance_log.append(sys_utt)
            file_name = "../../user_data/{0}/voice/{0}_{1}_voice"\
                .format(self.user, str(self.current_turn).zfill(3))
            self.record_log(sys_utt)
            ...
            sys_start_time = time.time() - self.base_time
            self.mmd.say(sys_utt_yomiage)
            self.record_log("*")
            self.record.start(file_name)
            self.asr.start()#音声認識スタート(./google_asr.py)
            while True:
                if self.asr.m_turn.is_sys_turn_end == True:#音声認識終了判定(./manage_turn.pyで判定)
                    break
                else:
                    time.sleep(0.1)
            self.record.stop()
            user_turn_end_time = time.time() - self.base_time
            user_utt,self.confident_noun = self.asr.read_result()
            self.env.user_utterance_log.append(user_utt)#対話ログは発話選択部(command.txtの最初のQテーブルフォルダ(sports))に保存(ここのログは使わない方が良い)
            user_utt_start_time = self.asr.get_utt_start_time()
            # 録音したwavファイルが保存されるまで待機
            while True:
                if self.record.save_complete == True:
            ...
            # 値のrange変更
            self.user_impression = pf.changePredValueRange(self.user_impression)
            print("心象:",self.user_impression)
            self.print_debug("user_impression:{}".format(self.user_impression))
            
            # 状態更新
            n_state = self.env.get_next_state(self.user_impression, sys_utt, user_utt)
            self.state = n_state
            self.custom_sys_utt_generator.set_input(user_utt, self.user_impression, self.confident_noun)
...
def main():
    ...
    parser.add_argument(
        ...
        dest = "is_custom",
        default = True,
        help = "発話選択手法をカスタマイズするか(要個別実装)(default:True)"
    )
    ...
    nh = NHChat(model=args.model, mode=args.mode, user=args.user,
                ...
                is_debug=args.is_debug, is_custom=args.is_custom)
