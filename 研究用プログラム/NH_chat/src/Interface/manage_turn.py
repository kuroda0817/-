...
class ManageTrun:
    ...
    def __init__(self, asr, response_time_no_word, turn_buffer, is_debug=False):
        ...
        ----------
        asr : instance
            JuliusASRかgoogleASRのインスタンスを渡す(ManageTrun(self)の形)
    ...
    def turn_take_simple_rule(self):
        ...
        """
        while True:
            if (self.asr.utt_start_time == None):
            ...
            if (elapsed_from_start > self.response_time_no_word) and \
                (self.asr.recognition_result == ""):
                self.print_debug("### turn: no word ###")
                break
            # 音声認識確定から一定時間(turn_buffer)経過でターンテイキング
            elif (elapsed_from_conf > self.turn_buffer) and \
                    (self.asr.recognition_result != ""):
                break
            time.sleep(0.1)
        self.asr.stop()
